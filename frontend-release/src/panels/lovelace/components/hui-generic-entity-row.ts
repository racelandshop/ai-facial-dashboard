...
class HuiGenericEntityRow extends LitElement {
  ...
  @property({ type: Boolean }) public static cssSize = false;
  ...
  protected render(): TemplateResult {
    ...
    return html`
      ...
      ${!this.config.hideInteract // aqui remove toggs
        ? this.catchInteraction ?? !DOMAINS_INPUT_ROW.includes(domain) // aqui toggles
          ? html` <div
              class="text-content2 ${classMap({
                pointer,
              })}"
              @action=${this._handleAction}
              .actionHandler=${actionHandler({
