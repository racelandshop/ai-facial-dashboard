...
import { showZonesDialog } from "./helpers/show-zones-dialog";
...
declare global {
  ...
  interface HASSDomEvents {
    "zones-opened": { zonesOpened: boolean };
  }
}
...
class HUIRoot extends LitElement {
  ...
  @state() private zonesOpened?: boolean = false;

  @query("ha-app-layout", true) private _appLayout!: haAppLayout;

  @query("#button-menu") private buttonMenu: any;
  ...
  constructor() {
    ...
    window.addEventListener("zones-opened", (ev) => {
      this.zonesOpened = ev.detail.zonesOpened;
    });
  }

  protected render(): TemplateResult {
    // console.log("xxxx", this.lovelace!.config.views[this._curView].title);

    return window.matchMedia("only screen and (min-width: 1500px)").matches // screen.width > 1500 is desktop
      ? html`
          <ha-app-layout
            class=${classMap({
            ...
            <app-header slot="header" effects="waterfall" fixed condenses>
              <app-toolbar>
                ...
                <div class="headtabs">
                  ...
                  <ha-tabtop
                    id="tabtop-zones"
                    style="padding: 0; width: 64px; font-size: 0.9rem;"
                    .hass=${this.hass}
                    .active=${true}
                    .narrow=${true}
                    .name=${this.hass!.localize("ui.panel.lovelace.menu.zones")}
                    @click=${this._showZones}
                  >
                    <ha-svg-icon
  ...
  protected updated(changedProperties: PropertyValues): void {
    ...
    if (this.buttonMenu) {
      const mwcMenu = this.buttonMenu.shadowRoot?.querySelector("mwc-menu");
      if (mwcMenu) mwcMenu.style.display = "none";
    }
    ...
    if (changedProperties.has("lovelace")) {
      ...
      // update zones dialog
      if (this.zonesOpened) {
        fireEvent(this, "lovelace-root-changed", {
  ...
  private get title(): string {
    return this.lovelace!.config.views[this._curView!]?.title
      ? this.lovelace!.config.views[this._curView!]?.title
      : "";
  }
  ...
  private _showZones() {
    // this.zonesOpened = true;
    showZonesDialog(this, {
      lovelace: this.lovelace!,
      viewIndex: this._curView as number,
      route: this.route,
    });
    ...
    return [
      ...
      css`
        ...
        .edit-mode app-toolbar {
          background-color: var(--app-header-edit-background-color);
