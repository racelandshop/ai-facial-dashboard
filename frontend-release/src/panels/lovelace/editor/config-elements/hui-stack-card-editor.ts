...
declare global {
  ...
  interface HASSDomEvents {
    "handle-selected-card": { cardId: number };
  }
}
  ...
  implements LovelaceCardEditor
{
  ...
  protected firstUpdated() {
    window.addEventListener("handle-selected-card", (ev) => {
      this._handleSelectedCardV2(ev.detail.cardId);
    });
  }
  ...
  protected _handleSelectedCard(ev) {
    ...
    fireEvent(this, "card-selected", { cardId: this._selectedCard });
  }

  protected _handleSelectedCardV2(index) {
    this._setMode(true);
    this._guiModeAvailable = true;
    this._selectedCard = parseInt(index, 10);
    fireEvent(this, "card-selected", { cardId: this._selectedCard });
  }
