    ...
    const schema = [
      ...
      },
      {
        ...
        schema: [
          ...
          { name: "unit", selector: { text: { max: 10 } } },
        ],
      },
      // { name: "theme", selector: { theme: {} } },
      {
        ...
        type: "grid",
        schema: [
          { name: "min", selector: { number: { min: 0, mode: "box" } } },
  ...
  private _valueChanged(ev: CustomEvent): void {
    ...
    if (config.show_severity) {
      config = {
        ...
        severity: {
          green:
            config.green || config.green === 0
              ? config.green
              : config.severity?.green,
          yellow:
            config.yellow || config.yellow === 0
              ? config.yellow
              : config.severity?.yellow,
          red:
            config.red || config.red === 0 ? config.red : config.severity?.red,
        },
      };
