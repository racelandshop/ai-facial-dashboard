...
declare global {
  ...
  interface HASSDomEvents {
    ...
    "card-selected": { cardId: number }; // declarar a funçao para a add + remove card
  }
  ...
  implements HassDialog<EditCardDialogParams>
{
      ...
      >
        ...
        ${false // this._cardConfig !== undefined
          ? html`
            ...
            `
          : ""}
        <div slot="secondaryAction" @click=${this._cancel} id="actions-cancel">
          <mwc-button id="cancelButton" @click=${this._cancel}>
            ${this.hass!.localize("ui.common.cancel")}
          </mwc-button>
        </div>
        <div slot="primaryAction" @click=${this._save} id="actions-save">
  ...
  private _updateSelected(e: any): void {
    if (e.detail != null) {
      this.selectedCard = e.detail.cardId;
      this.updateComplete.then(() => {
        const prevElem: any =
          this.shadowRoot!.querySelector("hui-card-preview");
        prevElem.selectedCard = this.selectedCard;
      });
    }
    console.log("estou aqui", this.selectedCard); // event listener
    // this.requestUpdate();
  }
  ...
  private _opened() {
    this.addEventListener("card-selected", this._updateSelected);
