...
class HuiGridCard extends HuiStackCard<GridCardConfig> {
  ...
  setConfig(config: GridCardConfig) {
    ...
    this.style.setProperty("--grid-card-column-count", String(this.columns)); // TODO: alterar aqui para nome_do_dict[String(this.columns)]

    // TODO: alterar de "1" para "grande" em localize   this.hass!.localize(`ui.panel.lovelace.editor.card.grid.big`);
    if (this.square && this._config?.columns !== "1") {
